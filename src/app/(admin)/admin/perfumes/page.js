...
export default function PerfumesListPage() {
  ...
  const handleToggleBestSeller = async (id, name, current) => {
    try {
      const res = await fetch(`/api/admin/perfumes/${id}/bestseller`, {
        method: "PATCH",
      });
      const data = await res.json();
      ...
      success(
        data.isBestSeller
          ? `"${name}" marked as best seller`
          : `"${name}" removed from best sellers`
      );
      fetchPerfumes();
    } catch (err) {
      error(err.message);
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          <svg
                            className="w-5 h-5"
                            viewBox="0 0 20 20"
                            fill={perfume.isBestSeller ? "currentColor" : "none"}
                            stroke="currentColor"
                            strokeWidth={perfume.isBestSeller ? "0" : "1.5"}
                          >
