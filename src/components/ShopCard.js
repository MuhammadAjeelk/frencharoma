...
export default function ShopCard({ name, image, href, tagline }) {
  return (
    <Link
      href={href}
      className="group relative block w-full max-w-[320px] rounded-2xl overflow-hidden border-2 border-[var(--fa-gold)] hover:border-[var(--fa-gold-hover)] shadow-[0_10px_30px_rgba(0,0,0,0.35)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500"
    >
      <div className="relative w-full aspect-[4/5]">
        ...
        <div className="absolute inset-x-0 bottom-0 px-5 pb-5 text-left">
          <h3 className="font-[family-name:var(--font-playfair)] text-2xl sm:text-3xl font-semibold tracking-wide text-[var(--fa-gold)] transition-colors duration-500 drop-shadow-[0_2px_8px_rgba(0,0,0,0.65)]">
            {name}
          </h3>

          {/* Line grows from the left; tagline sits centered on the line */}
          <div className="relative mt-2.5 h-4">
            <span className="pointer-events-none absolute left-0 top-1/2 -translate-y-1/2 h-[2px] bg-[var(--fa-gold)] w-9 group-hover:w-full transition-[width] duration-[650ms] ease-out" />
            {tagline && (
              <span className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 px-2.5 py-0.5 rounded bg-black/45 backdrop-blur-[2px] text-[var(--fa-gold)] text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.22em] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-200 drop-shadow-[0_2px_6px_rgba(0,0,0,0.6)]">
                {tagline}
              </span>
            )}
          </div>
