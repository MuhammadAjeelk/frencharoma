...
PerfumeSchema.pre("save", function (next) {
  // Check if at least one edition is enabled
  const enabledEditions = this.editions.filter((e) => e.enabled);

  if (this.status === "active" && enabledEditions.length === 0) {
    return next(new Error("Active perfumes must have at least one enabled edition"));
  }

  // For each enabled edition, check if it has at least one active variant
  for (const edition of enabledEditions) {
    ...
    // Check for duplicate sizes within the edition
    const sizes = edition.variants.map((v) => v.size);
    const uniqueSizes = new Set(sizes);
    if (sizes.length !== uniqueSizes.size) {
      return next(
        new Error(`Edition "${edition.key}" cannot have duplicate sizes`)
      );
    }

    // Validate active variants have price > 0
